...
export default function collisionTriAABB( a: ConstVec3, b: ConstVec3, c: ConstVec3, minBox:ConstVec3, maxBox:ConstVec3 ): boolean{
    let p0: number, p1: number, p2: number, r: number;
    ...
    const v0 = Vec3.sub( a, center ),
          v1 = Vec3.sub( b, center ),
          v2 = Vec3.sub( c, center );

    // Compute edge vectors for triangle
    const f0 = Vec3.sub( v1, v0 ),
          f1 = Vec3.sub( v2, v1 ),
          f2 = Vec3.sub( v0, v2 );

    // Test axes a00..a22 (category 3)
    const a00 = [      0, -f0[2],  f0[1] ],
          a01 = [      0, -f1[2],  f1[1] ],
          a02 = [      0, -f2[2],  f2[1] ],
          a10 = [  f0[2],      0, -f0[0] ],
          a11 = [  f1[2],      0, -f1[0] ],
    ...
    */
    ...
    let i   : any, 
        aa  : number, // First Axis Component
        bb  : number, // Second Axis Component
        ann : TVec3,  // Axis Point
        fn  : TVec3;  // Triangle Vector Length Edges
    ...
    if( Math.max( v0.x, v1.x, v2.x ) < -extents.x || Math.min( v0.x, v1.x, v2.x ) > extents.x ) return false;

    // ... [-extents.y, extents.y] and [min(v0.y,v1.y,v2.y), max(v0.y,v1.y,v2.y)] do not overlap
    if( Math.max( v0.y, v1.y, v2.y ) < -extents.y || Math.min( v0.y, v1.y, v2.y ) > extents.y ) return false;

    // ... [-extents.z, extents.z] and [min(v0.z,v1.z,v2.z), max(v0.z,v1.z,v2.z)] do not overlap
    if( Math.max( v0.z, v1.z, v2.z ) < -extents.z || Math.min( v0.z, v1.z, v2.z ) > extents.z ) return false;
