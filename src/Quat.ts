...
export default class Quat extends Float32Array{
    ...
    fromAngularVec( v: TVec3 ) : Quat{
        let len = Vec3.len( v );
        if( len < 0.000001 ){ this.reset(); return this; }
    ...
    rotDeg( deg: number, axis="y" ) : Quat{
        const rad = deg * Math.PI / 180;
        switch( axis ){
            case "x" : this.rotX( rad ); break;
            case "y" : this.rotY( rad ); break;
            case "z" : this.rotZ( rad ); break;
        }
        return this;
    }
    ...
    static lenSqr( a: TVec4, b: TVec4 ) : number{ return (a[0]-b[0]) ** 2 + (a[1]-b[1]) ** 2 + (a[2]-b[2]) ** 2 + (a[3]-b[3]) ** 2; }
    // #endregion
}